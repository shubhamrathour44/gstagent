...
from database import FirmRepo, UserRepo, get_db
...
def verify_token(token: str) -> dict:
    try:
        ...
        expected_sig = hmac.new(SECRET_KEY.encode(), f"{header}.{body}".encode(), hashlib.sha256).digest()
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f"Invalid token: {exc}", headers={"WWW-Authenticate": "Bearer"})
...
def _token_for(user, firm) -> TokenResponse:
    ...
    return TokenResponse(access_token=token, firm_id=firm.id, firm_name=firm.name, user_id=user.id, user_name=user.name, role=user.role)
...
def require_role(*roles: str):
    async def _check(current_user: CurrentUser = Depends(get_current_user)):
        if current_user.role not in roles:
            raise HTTPException(status_code=403, detail=f"Role '{current_user.role}' is not authorised")
        return current_user
    return _check
...
async def register_firm(request: RegisterFirmRequest, db: AsyncSession = Depends(get_db)):
    ...
    firm = await FirmRepo.create(db, name=request.firm_name.strip(), email=request.email, phone=request.phone, city=request.city)
    user = await UserRepo.create(db, firm_id=firm.id, email=request.email, name=request.admin_name.strip(), hashed_password=hash_password(request.password), role="ca_admin")
...
async def invite_staff(request: InviteStaffRequest, current_user: CurrentUser = Depends(require_role("ca_admin")), db: AsyncSession = Depends(get_db)):
    if await UserRepo.get_by_email(db, request.email):
        raise HTTPException(status_code=409, detail="User already exists")
    temp_password = base64.b64encode(os.urandom(9)).decode()[:12]
    await UserRepo.create(db, firm_id=current_user.firm_id, email=request.email, name=request.name, hashed_password=hash_password(temp_password), role=request.role)
    return {"message": f"Staff user created for {request.email}", "temp_password": temp_password, "note": "Send this password securely and force change later."}
...
async def change_password(request: ChangePasswordRequest, current_user: CurrentUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):

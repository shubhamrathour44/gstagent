...
router = APIRouter(prefix="/tally-import", tags=["Tally Import"])
...
def _normalize(row: dict, client_id: str, invoice_type: str) -> dict:
    ...
    invoice_number = str(_find(row, ["invoice_number", "invoice_no", "voucher_no", "voucher_number", "bill_no", "reference_no", "ref_no", "document_number"], f"TALLY-{uuid.uuid4().hex[:8].upper()}")).strip()
...
async def import_invoices(
    ...
    for _, r in df.iterrows():
        ...
        if len(samples) < 5:
            samples.append(inv)
    await db.commit()
    return {"status": "success", "provider": "tally", "imported": imported, "skipped": skipped, "samples": samples}

@router.post("/masters")
async def import_masters(file: UploadFile = File(...), current_user: CurrentUser = Depends(get_current_user)):
    df = await _read_table(file)
    return {"status": "success", "provider": "tally", "masters_found": len(df), "columns": list(df.columns)}
